...
import de.avkterwey.emoar.env.SplitTimer;
 ...
 */
public class TensorflowYoloDetector implements IClassifier {
    ...
    public List<Recognition> recognizeImage(final Bitmap bitmap) {
        ...
        for (int i = 0; i < intValues.length; ++i) {
            ...
            floatValues[i * 3 + 0] = ((intValues[i] & 0xFF)/225f - 0.485f) / 0.229f;
        }
            ...
            for (int x = 0; x < gridWidth; ++x) {
                for (int b = 0; b < NUM_BOXES_PER_BLOCK; ++b) {
                    ...
                    if (confidenceInClass > 0.01) {
                        //LOGGER.i("%s (%d) %f %s", LABELS[detectedClass], detectedClass, confidenceInClass, rect);
